...
if(!$_REQUEST['ajaxCall']) {
	...
	if($action == 'ADD') {
		...
		// Rename Image
		if(strpos($formData['svl_picture'], 'temp_') !== FALSE) {
			$type		= str_replace(".", "", strrchr($formData['svl_picture'],"."));
			$tmpRecord	= new TableSpa('service_lists', null);
			$svl_picture	= $tmpRecord->genKeyCharRunning().".$type";
			$svl_picture_path = '../img/service_lists/'.$svl_picture;

			// Delete Old Image
			if(file_exists($svl_picture_path)) {
				if(!unlink($svl_picture_path)) {
			...
			if(rename('../img/temp/'.$formData['svl_picture'], $svl_picture_path)) {
				$formData['svl_picture'] = $svl_picture;
			} else {
		...
		if(strpos($formData['svl_picture'], 'temp_') !== FALSE) {
			// Delete Old Image
			$oldImg = '../img/service_lists/'.$tableRecord->getFieldValue('svl_picture');
			...
			$type		= str_replace(".", "", strrchr($formData['svl_picture'],"."));
			$svl_picture	= $code.".$type";
			if(rename('../img/temp/'.$formData['svl_picture'], '../img/service_lists/'.$svl_picture)) {
				$formData['svl_picture'] = $svl_picture;
			} else {
				$response['status'] = 'RENAME_FAIL';
				echo json_encode($response);
