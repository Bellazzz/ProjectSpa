...
class DBRecord {
    ...
    public function __construct($key, $values) {
		global $table;
		$this->keyFieldName = $table[$this->tableName]['keyFieldName'];
		$this->key = $key;
    	$this->dbConnect();
		...
		if($key != null) {
			if(!is_array($key)) {
				...
				switch($table[$this->tableName]['keyFieldType']) {
					case 1:
	...
	public function insertAllField($pk, $values) {
		global $table;
		$allFieldList = $table[$this->tableName]['fieldNameList'];
	...
	public function genKeyCharRunning() {
		global $table;
		$keyLength	= $table[$this->tableName]['keyLength'];
		$keyChar	= $table[$this->tableName]['keyChar'];
	...
	public function keyIsChar() {
		global $table;
		if($table[$this->tableName]['keyFieldType'] == 1) {
			return false;
		} else {
			return true;
		}
